...
osc_rates <- c(0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0)
stim_codes <- c(3, 4) # 4
g_Cas <- c("0.028e-3", "0.029e-3", "0.031e-3", "0.032e-3", "0.033e-3")
...
for(i in 1:length(osc_codes)){
  for(s_c in stim_codes){
    for(g_ca in g_Cas){
      ...
      df$synapse <- rep("AMPA", nrow(df))
      df$homeo <- rep("No homeostasis", nrow(df))
      df$g_ca <- rep(g_ca, nrow(df))
      results274 <- rbind(results274, df)
    }
  }
}
results274 %>%
  filter(rate_k > 0) -> results274
...
for(i in 1:length(osc_codes)){
  for(s_c in stim_codes){
    for(g_ca in g_Cas){
      ...
      df$homeo <- rep("Homeostasis", nrow(df))
      df$g_ca <- rep(g_ca, nrow(df))
      results275 <- rbind(results275, df)
    }
  }
}
results275 %>%
  filter(rate_k > 0) -> results275

results <- rbind(results274, results275)
```

```{r, fig.width=4, fig.height=1.2}
results %>% 
  ggplot(aes(x=osc_rate, y=kappa_coord, group=interaction(osc_rate, g_ca), color=g_ca)) +
  geom_boxplot() +
  theme_classic() +
  scale_color_grey() +
...
```{r, fig.width=4, fig.height=1.2}
results %>% 
  ggplot(aes(x=osc_rate, y=rate_k, group=interaction(osc_rate, g_ca), color=g_ca)) +
  geom_boxplot(size=.5) +
  theme_classic() +
  scale_color_grey() +
  facet_grid(.~homeo, scales = "free")
