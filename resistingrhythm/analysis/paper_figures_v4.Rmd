...
results_fi %>% 
ggplot() +
  geom_line(aes(x=impulse*us, y=rate_ref/win/N, color="No homeostatis"), size=0.8) +
  geom_line(aes(x=impulse*us, y=rate_h/win/N, color="Homeostatis"), size=0.8) +
  labs(tag="a.", y="Avg. rate", x=TeX("Impulse ($\\mu$siemens)")) +
  lims(x=c(0, 10)) +
...
png(file="fig3.png", bg="white", width=3.8, height=1.8, unit="in", res=300)
print(p1)
dev.off()
```
...
png(file="fig4.png", bg="white", width=5.6, height=4.2, unit="in", res=300)
grid.arrange(p1, p2, pn,layout_matrix=layout_matrix)
dev.off()
```
